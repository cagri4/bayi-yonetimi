/**
 * Telegram Webhook Route — /api/telegram/muhasebeci
 *
 * Dedicated webhook for the Muhasebeci (Accountant) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_MUHASEBECI env var for the Muhasebeci bot
 * 2. Passes 'muhasebeci' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
 ...
 *
 * MH-06: Hallucination prevention enforced via system prompt KRITIK KURAL.
...
export async function POST(request: Request): Promise<Response> {
  ...
  // forcedRole='muhasebeci' routes to Muhasebeci tool set without agent_definitions lookup.
