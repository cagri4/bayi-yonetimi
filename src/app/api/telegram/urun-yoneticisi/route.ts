/**
 * Telegram Webhook Route — /api/telegram/urun-yoneticisi
 *
 * Dedicated webhook for the Urun Yoneticisi (Product Manager) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_URUN_YONETICISI env var for the Urun Yoneticisi bot
 * 2. Passes 'urun_yoneticisi' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/urun-yoneticisi] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='urun_yoneticisi' routes to Urun Yoneticisi tool set without agent_definitions lookup.
  const botToken = process.env.TELEGRAM_BOT_TOKEN_URUN_YONETICISI || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'urun_yoneticisi', botToken)
    } catch (err) {
      console.error('[telegram/urun-yoneticisi] dispatch error:', err)
    }
  })
