/**
 * Telegram Webhook Route — /api/telegram/satin-alma
 *
 * Dedicated webhook for the Satin Alma Sorumlusu (Purchasing) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_SATIN_ALMA env var for the Satin Alma bot
 * 2. Passes 'satin_alma' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/satin-alma] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='satin_alma' routes to Satin Alma tool set without agent_definitions lookup.
  const botToken = process.env.TELEGRAM_BOT_TOKEN_SATIN_ALMA || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'satin_alma', botToken)
    } catch (err) {
      console.error('[telegram/satin-alma] dispatch error:', err)
    }
  })
