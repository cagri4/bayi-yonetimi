/**
 * Telegram Webhook Route — /api/telegram/tahsilat-uzmani
 *
 * Dedicated webhook for the Tahsilat Uzmani (Collections) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_TAHSILAT_UZMANI env var for the Tahsilat Uzmani bot
 * 2. Passes 'tahsilat_uzmani' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/tahsilat-uzmani] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='tahsilat_uzmani' routes to Tahsilat Uzmani tool set without agent_definitions lookup.
  const botToken = process.env.TELEGRAM_BOT_TOKEN_TAHSILAT_UZMANI || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'tahsilat_uzmani', botToken)
    } catch (err) {
      console.error('[telegram/tahsilat-uzmani] dispatch error:', err)
    }
  })
