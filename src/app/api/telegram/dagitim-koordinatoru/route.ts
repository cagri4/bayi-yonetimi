/**
 * Telegram Webhook Route — /api/telegram/dagitim-koordinatoru
 *
 * Dedicated webhook for the Dagitim Koordinatoru (Distribution Coordinator) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_DAGITIM_KOORDINATORU env var for the Dagitim Koordinatoru bot
 * 2. Passes 'dagitim_koordinatoru' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/dagitim-koordinatoru] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='dagitim_koordinatoru' routes to Dagitim Koordinatoru tool set without agent_definitions lookup.
  const botToken = process.env.TELEGRAM_BOT_TOKEN_DAGITIM_KOORDINATORU || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'dagitim_koordinatoru', botToken)
    } catch (err) {
      console.error('[telegram/dagitim-koordinatoru] dispatch error:', err)
    }
  })
