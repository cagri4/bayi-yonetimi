/**
 * Telegram Webhook Route — /api/telegram
 *
 * Thin adapter: parse, dedup, dispatch. No agent logic here.
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // Vercel Fluid Compute allows up to 300 seconds for after() callbacks.
  after(async () => {
    try {
