/**
 * Telegram Webhook Route — /api/telegram/egitimci
 *
 * Dedicated webhook for the Egitimci (Trainer) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_EGITIMCI env var for the Egitimci bot
 * 2. Passes 'egitimci' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  // forcedRole='egitimci' routes to Egitimci tool set without agent_definitions lookup.
