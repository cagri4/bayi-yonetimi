/**
 * Telegram Webhook Route — /api/telegram/depo-sorumlusu
 *
 * Dedicated webhook for the Depo Sorumlusu (Warehouse Manager) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_DEPO_SORUMLUSU env var for the Depo Sorumlusu bot
 * 2. Passes 'depo_sorumlusu' as forcedRole — no agent_definitions role lookup needed
 *
 * Note: URL path uses kebab-case (depo-sorumlusu) but the role enum uses
 * underscores (depo_sorumlusu) — this is intentional and consistent with
 * the satis_temsilcisi route pattern.
 *
 * Protocol:
 ...
 *
 * DS-03: Stock update confirmation enforced via system prompt before update_stock.
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/depo-sorumlusu] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='depo_sorumlusu' routes to Depo Sorumlusu tool set without agent_definitions lookup.
  // Note: URL uses kebab-case (depo-sorumlusu); role enum uses underscores (depo_sorumlusu).
  const botToken = process.env.TELEGRAM_BOT_TOKEN_DEPO_SORUMLUSU || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'depo_sorumlusu', botToken)
    } catch (err) {
      console.error('[telegram/depo-sorumlusu] dispatch error:', err)
    }
  })
