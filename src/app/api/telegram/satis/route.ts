/**
 * Telegram Webhook Route — /api/telegram/satis
 *
 * Dedicated webhook for the Satis Temsilcisi (Sales Representative) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_SATIS env var for the Satis Temsilcisi bot
 * 2. Passes 'satis_temsilcisi' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/satis] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='satis_temsilcisi' routes to Satis tool set without agent_definitions lookup.
  const botToken = process.env.TELEGRAM_BOT_TOKEN_SATIS || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'satis_temsilcisi', botToken)
    } catch (err) {
      console.error('[telegram/satis] dispatch error:', err)
    }
  })
