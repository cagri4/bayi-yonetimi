/**
 * Telegram Webhook Route — /api/telegram/pazarlamaci
 *
 * Dedicated webhook for the Pazarlamaci (Marketing) agent bot.
 * Identical protocol to the base /api/telegram route with two differences:
 * 1. Uses TELEGRAM_BOT_TOKEN_PAZARLAMACI env var for the Pazarlamaci bot
 * 2. Passes 'pazarlamaci' as forcedRole — no agent_definitions role lookup needed
 *
 * Protocol:
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (idempotencyError) {
    ...
    console.error('[telegram/pazarlamaci] idempotency insert error:', idempotencyError)
  }

  // Step 3: Schedule background dispatch via after()
  // dispatchAgentUpdate runs AFTER the 200 response is sent to Telegram.
  // forcedRole='pazarlamaci' routes to Pazarlamaci tool set without agent_definitions lookup.
  const botToken = process.env.TELEGRAM_BOT_TOKEN_PAZARLAMACI || ''

  after(async () => {
    try {
      await dispatchAgentUpdate(update, 'pazarlamaci', botToken)
    } catch (err) {
      console.error('[telegram/pazarlamaci] dispatch error:', err)
    }
  })
